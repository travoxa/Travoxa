...
import { foodPackages, FoodPackage } from '@/data/foodData';
import AOS from 'aos';
...
const FoodAndCafesPage = () => {
    ...
    // State
    const [filteredPackages, setFilteredPackages] = useState<FoodPackage[]>(foodPackages);
    // Search State
    ...
    const applyFilters = (query: typeof searchQuery, currentFilters: typeof filters) => {
        ...
        if (currentFilters.cuisine.length > 0) {
            results = results.filter(pkg => currentFilters.cuisine.includes(pkg.cuisine));
        }
