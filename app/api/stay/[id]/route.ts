    ...
    try {
        ...
        if (!id) {
            return NextResponse.json(
                { success: false, error: 'Stay ID or slug is required' },
                { status: 400 }
            );
        }
        ...
        stay = await Stay.findOneAndUpdate(
            { slug: id },
            { $inc: { views: 1 } },
            { new: true }
        )
        ...
        if (!stay && id.match(/^[0-9a-fA-F]{24}$/)) {
            stay = await Stay.findByIdAndUpdate(
                id,
                { $inc: { views: 1 } },
                { new: true }
            )
        ...
        const stayWithId = {
            ...JSON.parse(JSON.stringify(stay)),
