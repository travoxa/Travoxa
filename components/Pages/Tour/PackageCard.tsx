...
export default function PackageCard({ pkg, isBlurItem }: PackageCardProps) {
    ...
    const isVendor = pkg.vendorId && (pkg.vendorId as any).vendorDetails?.businessName;
    const creatorName = isVendor ? (pkg.vendorId as any).vendorDetails.businessName : "Travoxa Verified";
        ...
        <div
            ...
            className={`group bg-white rounded-2xl overflow-hidden border border-slate-200 transition-all duration-300 flex flex-col h-full
                ${isBlurItem
                    ? ''
                    : isVendor
                        ? 'hover:border-violet-500/50 cursor-pointer hover:shadow-xl hover:shadow-violet-500/10'
                        : 'hover:border-emerald-500/50 cursor-pointer hover:shadow-xl hover:shadow-emerald-500/10'
                }
            `}
        >
            {/* Image Section */}
            <div className="relative h-56 w-full overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2 items-start">
                    {/* Creator Badge */}
                    <div className={`px-2 py-1 rounded-md text-[9px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-sm flex items-center gap-1
                        ${isVendor
                            ? 'bg-violet-600/90 text-white'
                            : 'bg-emerald-600/90 text-white'
                        }`}
                    >
                        {!isVendor && <FaStar size={8} className="text-yellow-400" />}
                        {creatorName}
                    </div>

                    {pkg.earlyBirdDiscount && pkg.earlyBirdDiscount > 0 && (
                        <div className="bg-orange-500 text-white px-2 py-1 rounded text-[10px] font-medium uppercase tracking-wide shadow-sm">
                            {pkg.earlyBirdDiscount}% OFF
                        </div>
                    )}

                    {!pkg.earlyBirdDiscount && (
                        <div className="bg-slate-900/80 backdrop-blur-sm text-white px-2 py-1 rounded text-[10px] font-medium uppercase tracking-wide">
                            Tour Package
                        </div>
                    )}
                </div>
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                {/* Title */}
                <h3 className={`text-lg font-medium text-slate-900 mb-3 leading-tight transition-colors line-clamp-2 min-h-[3rem] Mont
                    ${isVendor ? 'group-hover:text-violet-600' : 'group-hover:text-emerald-600'}`}
                >
                    {pkg.title}
                </h3>
                ...
                <div className="mb-4 flex-grow">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        ...
                        {(itineraryActivities.length > 0 ? itineraryActivities : (pkg.highlights && pkg.highlights.length > 0 ? pkg.highlights : pkg.inclusions))?.slice(0, 3).map((item, idx) => (
                            <span key={idx} className={`text-[10px] px-2 py-0.5 rounded-full font-medium border line-clamp-1 max-w-full
                                ${isVendor
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 flex items-end justify-between">
                    <div>
                        ...
                        {minPricePax > 0 && (
                            <div className={`text-[10px] font-bold mt-0.5 ${isVendor ? 'text-violet-600' : 'text-emerald-600'}`}>
                                (for {minPricePax} Pax)
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            onClick={handleShare}
                            className={`w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-all shadow-sm
                                ${isVendor ? 'hover:bg-violet-600 hover:text-white' : 'hover:bg-emerald-600 hover:text-white'}`}
                        ...
                        <button
                            ...
                            className={`text-white text-xs font-medium px-4 py-2.5 rounded-full transition-colors shadow-none
                                ${isVendor ? 'bg-violet-600 hover:bg-violet-700' : 'bg-slate-900 hover:bg-emerald-600'}`}
                        >
                            View Details
                        </button>
                    </div>
