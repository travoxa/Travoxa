...
const Choose = () => {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-[80px] py-[80px] ">
      ...
      <div className="text-center mb-[72px] max-w-4xl mx-auto">
        <p className="Mont text-[18px] tracking-[4px] uppercase text-[#4da528] font-bold  mb-[12px]">
          WHY CHOOSE US
        </p>
        <h2 className=" text-[56px] lg:text-[68px] font-bold Mont leading-[1.1] mb-[20px] bg-gradient-to-r from-black to-gray-600 bg-clip-text text-transparent">
          Features We Offer
        </h2>
        <p className="Mont text-[22px] text-gray-600 font-light leading-[1.6] max-w-5xl mx-auto">
      ...
      <div className="flex flex-col gap-[24px] max-w-6xl mx-auto">
        {rows.map((row, rowIndex) => (
          <div key={rowIndex} className="flex gap-[24px] items-stretch">
            {row.map((feature, featureIndex) => {
              ...
              return (
                <div 
                  key={absoluteIndex}
                  className={`bg-white rounded-[12px] border border-gray-100 hover:shadow-xl hover:shadow-gray-200/60 hover:-translate-y-[3px] transition-all duration-300 flex flex-col group relative overflow-hidden ${
                    isBig 
                ...
                >
                  ...
                  {/* Title */}
                  <h3 className={`font-bold Mont leading-[1.2] text-gray-900 ${
                    isBig ? 'text-[26px]' : 'text-[20px]'
                  }`}>
                    {feature.title}
                  </h3>

                  {/* Description */}
                  <p className={`text-gray-600 leading-[1.5] font-light mt-[6px] ${
                  ...
                  <div className={`grid grid-cols-2 gap-x-[10px] gap-y-[6px] mt-[10px] ${
                    ...
                    {feature.bullets.slice(0, 4).map((bullet, bulletIndex) => (
                      <div key={bulletIndex} className="flex items-center gap-[5px]">
                        <span className={`text-[#4da528] font-bold flex-shrink-0 leading-none ${
                          isBig ? 'text-[14px]' : 'text-[12px]'
                        }`}>•</span>
                        <span className="text-gray-700 leading-[1.4] font-light">
                          {bullet}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="mt-auto pt-[12px] flex items-center justify-between border-t border-gray-100">
                    <button className={`bg-black text-white font-bold Mont rounded-full hover:bg-[#4da528] hover:shadow-lg hover:shadow-[#4da528]/30 transition-all duration-300 transform group-hover:scale-[1.02] ${
                      isBig ? 'text-[12px] px-[16px] py-[6px]' : 'text-[10px] px-[12px] py-[5px]'
                    }`}>
                      SEE MORE
                    </button>
                    ...
                    <div className="flex items-center gap-[4px] opacity-60 group-hover:opacity-100 transition-opacity duration-300">
                      <div className={`h-[2px] bg-[#4da528] rounded-full transform group-hover:w-[28px] transition-all duration-300 ${
                        isBig ? 'w-[24px]' : 'w-[18px]'
                      }`}></div>
      ...
      <div className="lg:hidden flex flex-col gap-[20px] max-w-3xl mx-auto mt-[48px]">
        {featureData.map((feature, index) => (
          ...
          >
            ...
            <p className="text-[13px] text-gray-600 mb-[10px] font-light leading-[1.5]">
              {feature.description}
            </p>
            ...
            <div className="flex flex-col gap-[6px] mb-[12px]">
              {feature.bullets.slice(0, 3).map((bullet, bulletIndex) => (
                <div key={bulletIndex} className="flex items-center gap-[5px]">
                  ...
                  <span className="text-[12px] text-gray-700 font-light">{bullet}</span>
                </div>
              ))}
            </div>
